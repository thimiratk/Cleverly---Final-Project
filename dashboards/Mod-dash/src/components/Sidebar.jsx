      ...
      <div className="sticky bottom-0 bg-blue-600 p-4 border-t border-blue-500/30">
        ...
        {sidebarOpen && (
          <div className="mb-3 p-2 bg-blue-700 rounded-lg">
            <p className="text-sm font-medium">{moderator?.name || 'Moderator'}</p>
            <p className="text-xs text-blue-200">{moderator?.username}</p>
            {moderator?.assignedCategories && moderator.assignedCategories.length > 0 && (
              <p className="text-xs text-green-200 mt-1">
                Categories: {moderator.assignedCategories.join(', ')}
              </p>
            )}
          </div>
        )}
